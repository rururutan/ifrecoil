...
static __declspec(thread) char spi_wide_filename[MAX_PATH];
...
int SpiOpenW(SPI_FILE *fp, LPCWSTR buf, LONG_PTR len, unsigned int flag)
{
	...
	case 0:			/* 入力がディスクファイル */
		...
		fp->flags   = SPI_IOTYPE_FILE;
		fp->fhandle = hFile;
		/* Recoil dispatches by extension; preserve the W entry-point filename. */
		WideCharToMultiByte(CP_ACP, 0, buf, -1, spi_wide_filename,
		                    sizeof(spi_wide_filename), NULL, NULL);
		fp->fname   = spi_wide_filename;
		fp->foffset = len;
		fp->mbuffer = pBuff;
		fp->mptr    = pBuff;
		fp->mcount  = 0;
...
LONG_PTR SpiGetFileSize(SPI_FILE *fp)
{
    ...
    case SPI_IOTYPE_FILE:
    {
        ...
        if (!GetFileSizeEx(fp->fhandle, &li))
            return -1;

        return (LONG_PTR)li.QuadPart;
    }

    case SPI_IOTYPE_MEMORY:
        return (LONG_PTR)fp->msize;
